...
uneven_sim_res <- function(r, nrep){
  ...
  table_with_prev <- foreach (rep=1:nrep, .combine = rbind)%dopar%{
    ...
    pig_alloc_c <- ssnma(p_CEFTP,p_TILD_c,sigma_prev,0.8)$sample_size
  ...
  write.csv(dat_para, file = paste0("./table/table2_uneven_lor_",r,".csv"), row.names = F)
